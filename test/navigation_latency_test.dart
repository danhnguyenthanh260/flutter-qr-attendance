...
import 'package:flutter_qr_attendance/views/attendance/attendance_view.dart';
import 'package:flutter_qr_attendance/views/session/session_selection_view.dart';
...
void main() {
  ...
  Widget buildTestApp({
    ...
    return MultiProvider(
      providers: [
        ...
        ChangeNotifierProvider<AttendanceResultsProvider>.value(value: resultsProvider),
        ChangeNotifierProvider<AttendanceHistoryProvider>.value(value: historyProvider),
      ],
  ...
  group('Issue #48 - Navigation State Preservation & Startup Latency Tests', () {
    testWidgets('Lazy IndexedStack mounts Tab 0 initially and mounts other tabs on demand', (tester) async {
      ...
      final sessionProvider = SessionProvider(service: service, storage: storage);
    ...
    testWidgets('Switching between tabs 10 rounds preserves view state with 0 reload', (tester) async {
      tester.view.physicalSize = const Size(1280, 800);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      expect(sessionProvider.selectedClass?.id, equals(initialSelectedClassId));
      expect(sessionProvider.selectedSlot, equals(initialSlot));
    });

    test('SessionProvider.loadInitialData deduplicates concurrent in-flight calls', () async {
      final service = MockAttendanceService(simulateDelay: false);
      final storage = MemorySessionStorage();
      final sessionProvider = SessionProvider(service: service, storage: storage);
    ...
    testWidgets('SessionSelectionView renders immediately without blocking full screen when loading', (tester) async {
      tester.view.physicalSize = const Size(1280, 800);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      expect(find.text('1. Chọn lớp học / Môn giảng dạy'), findsOneWidget);
      expect(find.text('2. Chọn ca học (Slot)'), findsOneWidget);
    });
  });
}
