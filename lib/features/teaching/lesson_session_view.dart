...
class LessonSessionView extends StatefulWidget {
  const LessonSessionView({super.key});
  @override
  State<LessonSessionView> createState() => _LessonSessionViewState();
}

class _LessonSessionViewState extends State<LessonSessionView> {
  ...
  Future<void> _start(SessionProvider provider) async {
    final date = provider.selectedSlot?.date;
    ...
    if (date != today) {
      final accepted = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          content: Text(
            'Bạn đang mở điểm danh cho ngày $date. Kết quả sẽ được ghi vào ngày này.',
          ),
          actions: [
            TextButton(
            ...
            FilledButton(
              ...
              child: const Text('Mở đúng buổi đã chọn'),
            ),
          ],
        ),
      );
                          ...
                          children: [
                            for (final slot in provider.slots)
                              ChoiceChip(
                                ...
                                label: Text(
                                  '${slot.date} · Ca ${slot.slotNumber} · ${slot.timeRange}',
                                ),
